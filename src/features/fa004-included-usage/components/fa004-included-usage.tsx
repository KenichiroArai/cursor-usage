...
const TABLE_COLUMNS = [
  ...
  { key: 'model', label: 'モデル' },
  { key: 'inputWithCache', label: 'Input (W/CACHE WRITE)' },
  { key: 'inputWithoutCache', label: 'Input (W/O CACHE WRITE)' },
...
function buildStackedBarChartData(
  ...
  if (data.length === 0) return { labels: [] as string[], datasets: [] };

  const { modelData, uniqueDates } = processModelData(data, fieldName);
...
export function Fa004IncludedUsage() {
  ...
  const diffTotalTokens =
    latest && prev
      ? calculateDifferenceWithReset(latest.totalTokens, prev.totalTokens)
      : null;
  const diffInputWithCache =
    latest && prev
      ? calculateDifferenceWithReset(latest.inputWithCache, prev.inputWithCache)
      : null;
  const diffInputWithoutCache =
    latest && prev
      ? calculateDifferenceWithReset(
          latest.inputWithoutCache,
          prev.inputWithoutCache
        )
      : null;
  const diffCacheRead =
    latest && prev
  ...
  const diffApiCost =
    latest && prev
      ? calculateDifferenceWithReset(
          parseFloat(String(latest.apiCost).replace(/[$,]/g, '')) || 0,
          parseFloat(String(prev.apiCost).replace(/[$,]/g, '')) || 0
        )
      : null;
  const diffCostToYou =
    latest && prev
      ? calculateDifferenceWithReset(
          parseFloat(String(latest.costToYou).replace(/[$,]/g, '')) || 0,
          parseFloat(String(prev.costToYou).replace(/[$,]/g, '')) || 0
        )
      : null;
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      (r) =>
        ...
        String(r.totalTokens).toLowerCase().includes(q)
    );
  }, [data, search]);
  ...
  const sortedData = useMemo(() => {
    ...
    return list.sort((a, b) => {
      const av = a[sortKey as keyof IncludedUsageRecord];
      const bv = b[sortKey as keyof IncludedUsageRecord];
      let cmp = 0;
      if (sortKey === 'dateStr') {
        cmp = a.date.getTime() - b.date.getTime();
      } else if (
        sortKey === 'inputWithCache' ||
        sortKey === 'inputWithoutCache' ||
        sortKey === 'cacheRead' ||
        sortKey === 'output' ||
        sortKey === 'totalTokens'
      ) {
  ...
  const totalTokensChartData = useMemo(
    () => buildStackedBarChartData(data, 'totalTokens', 'Total Tokens'),
    [data]
  );
  ...
  const inputWithCacheChartData = useMemo(
    () =>
      buildStackedBarChartData(data, 'inputWithCache', 'Input W/CACHE WRITE'),
    [data]
  );
  const inputWithoutCacheChartData = useMemo(
    () =>
  ...
  function getPreviousDayRecord(
    record: IncludedUsageRecord
  ): IncludedUsageRecord | null {
  ...
  return (
    <>
      ...
      <div className="container-fluid mt-3">
        ...
        <div className="dashboard-container mb-4">
          <div className="row g-3">
            <div className="col-md-6">
              <div className="card h-100">
                <div className="card-header">
                  <h2 className="card-title mb-0">最新の Included Usage</h2>
                </div>
                <div className="card-body">
                  <div className="usage-included-stats">
                        ...
                        <div className="usage-stats-summary">
                          ...
                          <div className="stat-item">
                            ...
                            <span className="stat-value">
                              ...
                              {diffTotalTokens != null && (
                                <small
                                  className={getDifferenceClass(diffTotalTokens)}
                                >
                                  {' '}
                                  ({formatDifference(diffTotalTokens)})
                                </small>
                              )}
                            </span>
                          </div>
                          <div className="stat-item">
                            ...
                            <span className="stat-value">
                              {latest?.inputWithCache.toLocaleString() ?? '-'}
                              {diffInputWithCache != null && (
                                <small
                                ...
                                >
                                  {' '}
                                  ({formatDifference(diffInputWithCache)})
                                </small>
                              )}
                            </span>
                          </div>
                          <div className="stat-item">
                            ...
                            <span className="stat-value">
                              {latest?.inputWithoutCache.toLocaleString() ??
                                '-'}
                              {diffInputWithoutCache != null && (
                                <small
                                  className={getDifferenceClass(
                                    diffInputWithoutCache
                                  )}
                                >
                                  {' '}
                                  ({formatDifference(diffInputWithoutCache)})
                                </small>
                              )}
                            </span>
                          </div>
            ...
            <div className="col-md-6">
              <div className="card h-100">
                <div className="card-header">
                  <h2 className="card-title mb-0">Total Tokens グラフ</h2>
                </div>
                <div className="card-body">
                  <div className="chart-container">
                    <Bar
                      data={{
                        labels: totalTokensChartData.labels,
                        datasets: totalTokensChartData.datasets,
                      }}
                      options={{
                        ...chartOptionsBase,
                        plugins: {
              ...
              <div className="card h-100">
                ...
                <div className="card-body">
                  <div className="chart-container">
                    <Bar
                      data={{
                        labels: inputWithCacheChartData.labels,
                        datasets: inputWithCacheChartData.datasets,
                      }}
                      options={{
                        ...chartOptionsBase,
                        plugins: {
              ...
              <div className="card h-100">
                ...
                <div className="card-body">
                  <div className="chart-container">
                    <Bar
                      data={{
                        labels: inputWithoutCacheChartData.labels,
                        datasets: inputWithoutCacheChartData.datasets,
                      }}
                      options={{
                        ...chartOptionsBase,
                        plugins: {
        ...
        <div className="card">
          <div className="card-header">
            <h2 className="card-title mb-0">Included Usage 一覧</h2>
          </div>
          <div className="card-body">
            <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
              <label htmlFor="fa004-page-size" className="mb-0">
                <select
                  id="fa004-page-size"
              ...
              <div className="d-flex align-items-center gap-2">
                <label htmlFor="fa004-search" className="mb-0">
                  検索:
                </label>
                <input
                  id="fa004-search"
                ...
                <tbody>
                  {paginatedData.map((row, idx) => {
                    ...
                    const renderCost = (
                      ...
                      const prev = prevVal
                        ? parseFloat(String(prevVal).replace(/[$,]/g, '')) ||
                          0
                        : undefined;
                      const diff =
                        prev != null
                    ...
                    return (
                      <tr key={`${row.dateStr}-${row.model}-${idx}`}>
                        ...
                        <td className="text-end">
                          {renderNum(
                            row.inputWithCache,
                            prevRecord?.inputWithCache
                          )}
                        </td>
                        <td className="text-end">
                          {renderNum(
                            row.inputWithoutCache,
                            prevRecord?.inputWithoutCache
                          )}
                        </td>
                        <td className="text-end">
                          {renderNum(row.cacheRead, prevRecord?.cacheRead)}
                        </td>
                        <td className="text-end">
                          {renderNum(row.output, prevRecord?.output)}
                        </td>
                        <td className="text-end">
                          {renderNum(row.totalTokens, prevRecord?.totalTokens)}
                        </td>
                        <td className="text-end">
                        ...
                        <td className="text-end">
                          {row.costToYou === '' ||
                          parseFloat(
                            String(row.costToYou).replace(/[$,]/g, '')
                          ) === 0
                            ? '0'
            ...
            <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-2">
              ...
              <nav aria-label="Included Usage 一覧のページネーション">
