...
const CSV_PATH = path.join(projectRoot, 'public', 'data', 'usage-events.csv');
...
function formatYmdSlashFromUtcMs(utcMs: number): string {
...
function computeTotals(
  ...
  for (const line of lines) {
    ...
    if (wantDaily) {
      // UTC日付基準
      const dayKey = formatYmdSlashFromUtcMs(
        Date.UTC(
          new Date(rowDateMs).getUTCFullYear(),
          new Date(rowDateMs).getUTCMonth(),
          new Date(rowDateMs).getUTCDate(),
          0,
          0,
          0,
          0
...
function printPerModel(perModel: Map<string, number>): void {
  console.log('\nモデル別コスト:');
  if (perModel.size === 0) {
...
async function main() {
  try {
    ...
    const { perDay, perModel, total } = computeTotals(
      lines,
      startUtcMs,
      endUtcMs,
      opts.daily
    ...
    if (opts.daily) {
      ...
      let cursor =
        startUtcMs != null
          ? Date.UTC(
              new Date(startUtcMs).getUTCFullYear(),
              new Date(startUtcMs).getUTCMonth(),
              new Date(startUtcMs).getUTCDate(),
              0,
              0,
              0,
              0
      ...
      const last =
        endUtcMs != null
          ? Date.UTC(
              new Date(endUtcMs).getUTCFullYear(),
              new Date(endUtcMs).getUTCMonth(),
              new Date(endUtcMs).getUTCDate(),
              0,
              0,
              0,
              0
