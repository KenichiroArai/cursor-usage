...
REM ===========================================
REM リリース自動化スクリプト（HTML+CSS+JavaScript版）
REM ===========================================

REM 前提条件：
REM - GitHub アカウントを持っていること
REM - リポジトリへのプッシュ権限があること
REM - 以下のツールやサービスが必要に応じて利用可能であること：
REM     - Git
REM     - GitHub CLI（オプション：プルリクエストの自動作成に必要）

REM 使用方法：
REM - Web画面上で作業ブランチ・リリースブランチ・バージョンを入力し、「リリース作成」ボタンを押すことで、
REM   自動化されたリリース手順（例：GitHub APIによるリリース作成等）が実行されます。

REM 機能：
REM - 入力されたバージョン番号でリリース作成を支援
REM - コミット状況やリモート同期状況の表示（API利用時）
REM - プルリクエスト作成機能（GitHub API or CLI連携時）
REM - タグ作成・反映の支援（可能な場合）

REM 注意事項：
REM - バージョン番号の先頭の「v」は省略可能（プログラム側で自動補完されます）
REM - プルリクエストのマージ操作は手動
REM - Webアプリケーションとして提供するため、ローカルgitコマンドやMaven等のJavaビルドツール連携は行いません
REM - 本UIはUTF-8で保存してください

REM ファイル形式に関する注意事項：
REM - このHTMLファイルはUTF-8で保存してください
REM - 改行コードはLF（Unix形式）推奨です
