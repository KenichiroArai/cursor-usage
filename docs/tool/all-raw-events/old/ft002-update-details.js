...
/**
 * Inputフォルダから最新の日付のusage-details-*.csvファイルを検索する
...
function findLatestInputCSVFile(inputDir) {
    try {
        ...
        const csvFiles = files
            .filter(file => file.startsWith('usage-details-') && file.endsWith('.csv'))
            .map(file => {
                const dateMatch = file.match(/usage-details-(\d{4}-\d{2}-\d{2})/);
                if (dateMatch) {
                    return {
                        file: file,
        ...
        if (csvFiles.length > 0) {
            ...
            console.log('Inputフォルダにusage-details CSVファイルが見つかりませんでした');
            return null;
        }
    } catch (error) {
...
function main() {
    ...
    const outputFile = path.join(dataDir, 'usage-details.csv');

    console.log('usage-details CSVファイルの更新を開始します...');
    ...
    if (inputFile) {
        ...
        if (inputData.length > 0) {
            ...
            if (isNewData(existingData, inputData)) {
                ...
                console.log('usage-details CSVファイルの更新が完了しました！');
